<nav x-data="{ open: false }" class="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    {{-- AWAL TAMBAHAN UNTUK PRIMARY NAVIGATION --}}
                    {{-- PAUTAN UNTUK SUPER ADMIN --}}
                    @if (Auth::check() && Auth::user()->role->value === \App\Enums\UserRole::SUPER_ADMIN->value)
                        <x-nav-link :href="route('superadmin.settings')" :active="request()->routeIs('superadmin.settings')">
                            {{ __('Super Admin Settings') }}
                        </x-nav-link>
                    @endif

                    {{-- PAUTAN UNTUK ADMIN --}}
                    @if (Auth::check() && Auth::user()->role->value === \App\Enums\UserRole::ADMIN->value)
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            {{-- AWAL TAMBAHAN UNTUK RESPONSIVE NAVIGATION --}}
            {{-- PAUTAN RESPONSIVE UNTUK SUPER ADMIN --}}
            @if (Auth::check() && Auth::user()->role->value === \App\Enums\UserRole::SUPER_ADMIN->value)
                <x-responsive-nav-link :href="route('superadmin.settings')" :active="request()->routeIs('superadmin.settings')">
                    {{ __('Super Admin Settings') }}
                </x-responsive-nav-link>
            @endif

            {{-- PAUTAN RESPONSIVE UNTUK ADMIN --}}
            @if (Auth::check() && Auth::user()->role->value === \App\Enums\UserRole::ADMIN->value)
                <x-responsive-nav-link :href="route('admin.manage')" :active="request()->routeIs('admin.manage')">
                    {{ __('Urus Staf (Admin)') }}
                </x-responsive-nav-link>
            @endif
            {{-- AKHIR TAMBAHAN UNTUK RESPONSIVE NAVIGATION --}}
